...
namespace ABMVantage_Outbound_API.Functions
{
    public class DashboardFunctionDailyAverageOccupancy
    {
        ...
        [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "application/json", bodyType: typeof(DashboardDailyAverageOccupancy), Summary = "Get Average Daily Occupancy", Description = "Gets the average parking facility occupancy for the given day, potentially filtered by facility, level and product.")]
        [OpenApiResponseWithoutBody(statusCode: HttpStatusCode.BadRequest, Summary = "Invalid Filter Parameters", Description = "Invalid FilterParameters")]
        [OpenApiResponseWithoutBody(statusCode: HttpStatusCode.MethodNotAllowed, Summary = "Validation exception", Description = "Validation exception")]
        [OpenApiParameter(name: "calculationDate", In = ParameterLocation.Query, Required = true, Type = typeof(DateTime), Summary = "The date used for the average occupancy calculation.", Description = "The calculation date for the requested average faily occupancy. For example, if the calculation date is 2021-11-01 then the average occupancy is based on the total occupied minutes and total available minutes for all parking spots for that day 2021-11-01")]
