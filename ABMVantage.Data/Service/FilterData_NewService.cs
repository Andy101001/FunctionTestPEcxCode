    ...
    public class FilterDataService_New : ServiceBase, IFilterDataService_New
    {
        ...
        public async Task<FilterData> GetFiltersData(ServiceLocations request)
        {
            ...
            try
            {
                ...
                result.Levels = rawData.Where(x => x.CustomerId == request.CustomerId && custBuList.Contains(x.BuCode)).Select(l => new LevelData { FacilityId = l.FacilityId, FacilityName = l.FacilityName, Id = l.LevelId, Level = l.Level }).ToList().Distinct();
